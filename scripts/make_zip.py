...
def  main():
  ...
  installer = "\installer\CPNewPlugin Installer.exe"
   
  if demo:
    installer = "\installer\CPNewPlugin Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\CPNewPlugin manual.pdf" 
  ]
  ...
  ZIPNAME = "CPNewPlugin-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "CPNewPlugin-v" + FULLVERSIONSTR + "-win-demo.zip"
